...
\begin{center}
\includegraphics[width=.5\textwidth]{penal_quad.png}
\captionof{figure}{\label{fig:regul_creneau} Régularisation de Moreau-Yosida d'une fonction créneau. }
\end{center}
\end{multicols}
\todo[noline]{Mettre la bonne figure}
\begin{itemize}
...
\begin{figure}
\centering
\subfloat[\label{fig:para_tronquee} Parabole tronquée {$J(u)=[ u^2-a^2 ]^+ $}]{\includegraphics[width=.49\textwidth]{penal_quad.png}}
\subfloat[\label{fig:regul_para_coupee} Régularisée de Moreau-Yosida d'une parabole tronquée.]{\includegraphics[width=.49\textwidth]{penal_quad.png}}
...
\\ \hline
\end{tabular}
\caption{\label{tab:optim2gris_pen_quad_reg}Tableau récapitulatif des optimisations réalisées sur 2 niveaux de gris, $\tau_S$ fixé à 197, avec pour pénalisation une parabole tronquée régularisée (\cf Figure REF page XXX ).}
\end{table}
\todo{ref fig}
...
Visiblement rien ne semble y faire : il y a toujours des valeurs de $\tau_N$ qui s'approche des bornes autorisées et les cas où l'on considère les images $[3,5,7]$ et $[3,7,9]$ fournissent $\tau_N >> \tau_S$ ce qui est aberrant\todo{Parle-t-on de l'ordre ici ?}.
La régularité de la pénalisation ne semblent donc pas en cause ici. Le problème semble venir d'ailleurs (\cf retour au chapitre dans la section \todo{REF section}  ).
