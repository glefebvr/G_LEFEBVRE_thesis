...
Soit~$\Omega$ le domaine d'intérêt, où la tumeur croît, et soit~$\partial\Omega$ son bord, qui est supposé aussi régulier que nécessaire. Le domaine~$\Omega$ est supposé strictement inclus dans le tissu --le foie-- qui est plus grand \correction{(sinon, notre modèle devrait aussi prendre en compte des contraintes physiques liées à la paroi du foie, contraintes que nous ne savons pas décrire qualitativement). Nous supposons donc que la tumeur n'atteint pas le  bord~$\partial\Omega$.} 
%\correction{(sinon la donnée au bord du domaine de calcul devient prépondérante, donnée que nous ne possédons pas de manière précise d'autant  plus que la notion de front tumoral reste encore très controversée)}.
...
Le terme $1+M$ traduit le fait que les traitements arrivent à la tumeur principalement\footnote{\correction{Un traitement administré en intraveineuse agit même sur les tumeurs non vascularisées. Une partie du traitement parvient donc toujours à la tumeur. A cet apport de traitement de base (considéré égal à 1), s'ajoute l'apport par voies sanguines ($M$).}} par le réseau sanguin. 
%\correction{Nous avons arbitrairement choisi de décomposer ce terme en deux composantes~: une partie (égale à 1) qui arrive toujours à la tumeur même si elle est non vascularisée à laquelle s'ajoute la partie acheminée via le réseau sanguin~$M$.}  
...
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{gamma_v2.png}
...
Le terme $1+M$ traduit le fait que plus le milieu est vascularisé, plus l'élimination de la nécrose est rapide\footnote{\correction{Les phagocytes (cellules qui éliminent la nécrose) circulent dans l'organisme à travers le réseau lymphatique et le réseau sanguin. Ainsi, nous avons choisi de distribuer l'élimination de la nécrose de la même manière : une élimination de fond de la nécrose (que l'on considère égale à 1), qui a toujours lieu, même sans vascularisation, 
 à laquelle s'ajoute une élimination par voie sanguines ($M$).}}.
%Notez que b
...
Il a été signalé dans~\cite{Holmes2007} que les cellules hypoxiques augmentent leur production de facteurs de croissance, tandis que les cellules hautement proliférantes n'ont pas besoin d'apports nutritifs supplémentaires. Ainsi, si~$M$ est en dessous du seuil~$\Ms$ alors~$\xi$ doit augmenter. \correction{Sans traitement antiangiogénique, on pourrait alors écrire l'équation suivante sur~$\xi$~:
\begin{equation}\label{eq_xi_no_treat}
\partial_t \xi = \alpha \int_\Omega \big( \epsilon_{\xi} + \gammapd(M) / \gamma_1  \big) P \textrm{\;d}\vecx - \lambda \xi,
\end{equation}
où le paramètre~$\lambda$ est le taux d'élimination des facteurs de croissance angiogénique et où $\alpha$ est l'excitabilité (ou la capacité) angiogénique de l'individu. L'intégrale représente la production moyenne de facteur de croissance. Elle est produite par la population $P$ avec un taux minimal~$\epsilon_{\xi}$ qui augmente selon le degré d'hypoxie $\gammapd(M)/\gamma_1$.} 
%Notez aussi que l
...
%, le paramètre~$\epsilon_{\xi}$ traduit la production minimale de facteurs de croissance angiogénique par les cellules cancéreuses.
...
\correction{(de part sa fonction qui traite l'ensemble des éléments provenant de la digestion). Ici on suppose que la vascularisation présente dans le foie fournit 2 fois plus de nutriments que nécessaire aux cellules dans un foie normal. Notons que la condition limite est en accord avec ceci, puisque sur le bord~$\partial \Omega$, on considère $S=1$.}
\end{myitemize}
...
\correction{On suppose que la divergence de la vitesse~$\vit$ est bornée ($L^\infty([0,T]\times \Omega)$) et que~$M$ l'est aussi.}  
%la vitesse~$\vit$ est régulière et que~$M$ est bornée. 
...
\correction{Notons que les deux propriétés que nous venons de montrer (positivité et conservation de la densité totale) garantissent la bornitude de chacune des densités de population $P_i$, $N$ et $S$.}
...
D'autres schémas d'ordre moins élevé ont aussi été essayés. Cependant la diffusion numérique de ces schémas est trop importante. Ceci conduit à une diffusion artificielle\footnote{\correction{La diffusion numérique va engendrer un aplatissement de la zone de transition entre le tissu sain et le c\oe ur de la tumeur. Ceci va augmenter son irrigation (puisque l'irrigation est portée par le tissu sain). Les apports nutritifs se trouvent alors modifiés.}} des nutriments dans la tumeur. La croissance tumorale est alors modifiée par cet artefact. La résolution précise du transport est par conséquent un point important. De plus, l'utilisation d'un schéma d'ordre élevé réduit, par le gain de convergence, les problèmes usuels liés à la résolution du problème non conservatif.
...
%%\footnote{\correction{La variable $\xi$ étant l'amplitude de la vitesse de transport de $M$.}~:
...
-200 à +200 (en associant ainsi à la valeur -200 la couleur noir et à +200 le blanc) et nous fixons arbitrairement\footnote{\correction{Les coefficients sont simplement pris de sorte à obtenir des images qui soit visuellement acceptable en comparaison aux images cliniques.}} les coefficients ci-dessus à 
...
Le pas de temps~$\Delta t$, \correction{qui varie au cours du temps, est calculé à chaque itération} en utilisant l'équation~\eqref{eq:dt}. 


%%Les paramètres numériques ont été choisis
\correction{L'ensemble des valeurs des paramètres du modèle (résumé dans la Table~\ref{table_param}) a été choisi} de sorte à reproduire l'évolution de l'aire tumorale. En particulier, nous n'essayons pas de recoller de manière précise à la géométrie, nous vérifions seulement que l'évolution spatiale soit plausible comparée aux images médicales. La manière dont ont été obtenus les paramètres \correction{(optimisation pour reproduire au mieux l'évolution de l'aire tumorale)} est abordée dans la section~\ref{sec:code_modeleEDP} de l'annexe~\ref{chap:structure_code}.  
%L'ensemble des paramètres est résumé dans la Table~\ref{table_param}. 
...
%%\corrections{Bien que la reproduction fidèle de la géométrie de la tumeur au cours du temps ne soit pas notre objectif premier, nous construisons une donnée initiale qui se veut ressemblante à la tumeur  réelle au jour 0.}
Nous soulignons que la donnée initiale est cruciale dans la croissance de la tumeur. 
%Afin de reproduire qualitativement la forme de la lésion au temps initial, 
%la condition initiale est choisie comme une ellipse perturbée. 
\correction{Dans notre cas, il s'agit d'en construire une de sorte à s'approcher de la forme réelle constatée sur les scanners tout en gardant une géométrie régulière. 
%%Ainsi, la condition initiale est choisie comme une ellipse perturbée. 
%%Plus précisément,
La condition initiale est ici choisie comme étant une fonction plateau dans laquelle la transition entre 0 et 1 s'opère de manière régulière ($\mathscr{C}^2$) par le biais d'un raccord basé sur une demi-période de cosinus. Le support de cette fonction est quant à lui choisi comme une ellipse perturbée (forme à laquelle on applique ensuite une rotation pour l'orienter dans le même sens que dans le cas réel). Une telle fonction est visualisable sur la Figure~\ref{fig:CI_Y2D}.}
...
La fonction~$Y$ définie par\footnote{Notons que si~$e=0$, alors~$Y$ est  invariant par rotation.}
...
\end{array}\right.
\end{equation}
\correction{La Figure~\ref{fig:CI_Y1D} illustre la construction de cette fonction (et de son raccord). La quantité $c$ intervient comme un multiplicateur des rayons. Sans ce coefficient, les rayons définissent la largeur du support de la fonction $Y$ (ce qui correspond au tracé en pointillé rouge). Avec ce coefficient, les rayons définissent directement l'aire tumorale, au sens de l'équation~\eqref{eq:airetot} via le seuil $\epsilon_{th}$. La fonction~$Y$}  
...
qui sont aussi résistantes au traitement~$\TS$. \correction{Pour \Nber, le choix des paramètres~$e=0.35$ et~$r_1=0.47$, $r_2=0.36$ fournit une géométrie acceptable\footnote{Acceptable du point de vue de nos objectifs. La condition initiale construite avec ces paramètres ressemble assez à la tumeur réelle, mais ce n'est en aucun cas une reproduction fidèle de la géométrie constatée sur les scanners. Ici, notre but premier n'est pas de reproduire cette géométrie. L'approximation réalisée ici nous suffit donc.}.} 

\begin{figure}
\subfloat[\label{fig:CI_Y1D}Section de la condition initiale (en $y=0$) représentative du comportement générale. Ici $e=0$ et $r_1=1.5\;cm$.]{\includegraphics[width=.49\textwidth]{CI/Yini_d.png}}\quad
\subfloat[\label{fig:CI_Y2D}Aspect spatial de la CI pour la simulation numérique lié à \Nber ($e=0.35, r_1=0.47, r_2=0.36$) -- Seule la région ${[2,4]^2}$ est présentée ici, la fonction étant nulle sur le reste du domaine.]{\includegraphics[width=.49\textwidth]{CI/Yini_ligne_de_nvx.png}}
\caption{\correction{Géométrie de la condition initiale (avec ici $L=D=6\;cm$)\label{fig:CI_Y}}}
\end{figure}
...
\end{figure}
\footnotetext{\correction{Le décalage de quelque jour entre les images cliniques et les images numériques est dû au fait que les sorties numériques (étant limitées pour ne pas alourdir le calcul numérique déjà  couteux) ne coïncident pas nécessairement avec les jours auxquels sont réalisés les examens médicaux.}}
...
\end{tabular}
\caption{\label{fig_chen_spatial} Evolution spatiale de la métastase hépatique de \Chen sur une série de scanners. Nous examinons l'évolution de la lésion\protect\footnotemark~située au départ en haut à gauche de la Figure~\ref{fig_chen_spatial_1}. Ici n'est présentée qu'une sélection de scanners~: l'ensemble complet est présenté dans l'annexe~\ref{chap:anx_img_complement}.}
\end{figure}
...
Il est important de noter que notre modèle reproduit %fite
