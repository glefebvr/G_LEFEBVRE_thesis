...
Soit~$\Omega$ le domaine d'intérêt, où la tumeur croît, et soit~$\partial\Omega$ son bord, qui est supposé aussi régulier que nécessaire. Le domaine~$\Omega$ est supposé strictement inclus dans le tissu --le foie-- qui est plus grand \correction{(sinon, notre modèle devrait aussi prendre en compte des contraintes physiques liées à la paroi du foie)}. De plus, nous supposons que la tumeur n'atteint pas le bord~$\partial\Omega$ \correction{(sinon la donnée au bord du domaine de calcul devient prépondérante, donnée que nous ne possédons pas de manière précise d'autant  plus que la notion de front tumoral reste encore très controversée)}.
...
\caption{Géométrie de la condition initiale (avec ici $L=D=6\;cm$)\label{fig:CI_Y}}
\end{figure}
